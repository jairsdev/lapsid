...
class Pagina {
    ...
    public function criar_tabela() {
        $query = "INSERT INTO conteudo_paginas (titulo, conteudo, data_criacao, data_atualizacao) VALUES (:titulo, :conteudo, :data_criacao, :data_atualizacao)";
    ...
    public function ler_registros() {
        $query = "SELECT * FROM conteudo_paginas ORDER BY id DESC";
        $stmt = $this->conn->prepare($query);
        $stmt->execute();
    ...
    public function ler_registro() {
        $query = "SELECT * FROM conteudo_paginas WHERE id = :id ORDER BY id DESC";
        $stmt = $this->conn->prepare($query);
    ...
    public function deletar_registro() {
        $query = "DELETE FROM conteudo_paginas WHERE id = :id";
        $stmt = $this->conn->prepare($query);
        $stmt->bindValue(":id", $this->id, PDO::PARAM_INT);
        if ($stmt->execute()) {
            return $stmt->rowCount();
